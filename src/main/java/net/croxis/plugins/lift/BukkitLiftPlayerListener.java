...
public class BukkitLiftPlayerListener implements Listener{
	...
	public void onPlayerInteract(PlayerInteractEvent event){
		if ((event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.LEFT_CLICK_BLOCK) 
			...
			plugin.logDebug("Sign Clicked.");
			plugin.logDebug(event.getClickedBlock().getType().toString());
			if (event.getClickedBlock().getType() == Material.OAK_WALL_SIGN || event.getClickedBlock().getType() == Material.BIRCH_WALL_SIGN || event.getClickedBlock().getType() == Material.SPRUCE_WALL_SIGN || event.getClickedBlock().getType() == Material.JUNGLE_WALL_SIGN || event.getClickedBlock().getType() == Material.ACACIA_WALL_SIGN || event.getClickedBlock().getType() == Material.DARK_OAK_WALL_SIGN 
                && buttonBlock != null
			    && BukkitElevatorManager.isButton(buttonBlock)) {
					plugin.logDebug("detected wallsign");
